let $PSInvasionMob;
try {
  $PSInvasionMob = Java.loadClass('dev.theagameplayer.puresuffering.world.entity.PSInvasionMob');
} catch (e) {
  // Ignored on client if not present
}
...
EntityEvents.death((event) => {
  ...
  if (entity.monster && source.actual && source.actual.isPlayer()) {
    ...
    if ($PSInvasionMob && entity.nbt[$PSInvasionMob.HYPER_CHARGE] > 0) {
      if (Math.random() < 0.25) {
        entity.block.popItem(Item.of('shadowlands:silver_coin', 1));
      }
      if (Math.random() > (1 - difficulty / (difficulty + 100)) && Math.random() < 0.5) {
        let coinCount = Math.max(1, Math.floor(entity.nbt[$PSInvasionMob.HYPER_CHARGE] / 3));
        entity.block.popItem(Item.of('shadowlands:silver_coin', coinCount));
      }
    }
